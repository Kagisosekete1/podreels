...
import { Heart, MessageCircle, UserPlus, Bell, Loader2, Mail, Send } from 'lucide-react';
      ...
      <div className="min-h-screen bg-background flex flex-col pb-20">
        <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border">
          <div className="flex items-center gap-3 h-14 px-4">
            <button onClick={() => setActiveChat(null)} className="text-sm text-primary font-medium">← Back</button>
          ...
          {chatMessages.map(m => (
            <div key={m.id} className={`flex ${m.sender_id === user.id ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] px-3 py-2 rounded-2xl text-sm ${
                m.sender_id === user.id ? 'gradient-primary text-primary-foreground' : 'bg-muted text-foreground'
              }`}>
                {m.content}
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border">
        ...
        <div className="flex border-b border-border">
          <button
            ...
            className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${tab === 'alerts' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`}
          >
            Alerts
          </button>
          <button
            onClick={() => setTab('inbox')}
            className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${tab === 'inbox' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground'}`}
          >
      ...
      {loading ? (
        ...
        conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-8">
            ...
            <p className="text-xs text-muted-foreground mt-1">Send a message from someone's profile to start a conversation</p>
          </div>
        ) : (
          <div className="divide-y divide-border">
            {conversations.map(c => (
